...
# Deploy extractBehavioralFeatures Lambda function

set -e

FUNCTION_NAME="mindmate-extractBehavioralFeatures"
REGION=${AWS_REGION:-us-east-1}
LAMBDA_DIR="backend/lambdas/extractBehavioralFeatures"
...
if [ -z "$ML_LAMBDA_ROLE_ARN" ]; then
    ...
    echo "Please deploy the ML infrastructure first: ./infrastructure/deploy-ml-stack.sh"
    exit 1
fi
